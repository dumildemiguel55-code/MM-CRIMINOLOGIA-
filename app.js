...
function tocarSomErro() {
  ...
  osc.frequency.linearRampToValueAtTime(110, agora + 0.25);
...
function carregarHistoricoAgentes() {
  ...
  const agentes = JSON.parse(localStorage.getItem("agentes_registados")) || [];

  if (agentes.length === 0) {
    tabelaBody.innerHTML = `
      <tr>
        <td colspan="4" style="text-align: center; color: #888; padding: 15px;">
          Nenhum agente registrado no histórico até o momento.
        </td>
      </tr>
    `;
    return;
